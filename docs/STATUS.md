...
- **Gestão de Serviços** (novo grupo no sidebar, 25/08): `/gestao-servicos/distribuicao` (Lista de Distribuição — funcional) · `/gestao-servicos/painel` (Painel do Gestor — placeholder "em construção") · `/gestao-servicos/precificacao` (Precificação — placeholder) · `/gestao-servicos/areas` (Config. Áreas — CRUD funcional).
...
- **Gestão de Serviços `dist_`:** `dist_areas`, `dist_colaborador_area` (uso futuro), `dist_servico`, `dist_servico_colaborador` — tabelas próprias (não replicadas do Firebird), criadas fora deste repo (pelo cérebro/Supabase MCP) antes da tela ser construída. Sem tipagem em `types.ts` (mesmo padrão do Tacógrafo — usa `db = supabase as any` em `src/lib/dist.ts`). Anon/authenticated têm todos os grants, sem RLS (padrão do projeto).
...
- [x] Gestão de Serviços — shell (sidebar/rotas) + Config. Áreas + Lista de Distribuição construídos e testados ponta a ponta (25/08). Lista roda hoje direto em cima de `dist_servico` (mock manual) porque a `vw_dist_servicos` ainda não foi publicada — Leo avisa quando publicar.
- [ ] Gestão de Serviços — Painel do Gestor e Precificação: hoje são placeholders "em construção". Painel (disponibilidade de colaborador) dá pra construir com `vw_serv_apontamentos_base` (já existe) sem depender da view nova; Precificação depende da `vw_dist_servicos` (precisa do `ok='S'` do legado pra achar finalizados).
- [ ] Gestão de Serviços — quando a `vw_dist_servicos` publicar: trocar a fonte de dados da Lista (hoje só mostra o que já foi lançado em `dist_servico`; a view vai trazer os ~678 serviços em aberto do legado, com cliente/placa/serviço/preço/tempo) e plugar o status automático (`em_servico`/`finalizado`, hoje `deriveStatusEfetivo()` em `src/lib/dist.ts` só cobre o lado manual).
